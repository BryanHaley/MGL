...
"}\n"
"\n"
"struct GatherInfo { uint stride; uint dstOffset; uint count; };\n"
"\n"
"// one recorded vertex per thread, taken from the slot the list names\n"
"kernel void gatherVertices(device const uint32_t* src [[ buffer(0) ]],\n"
"                           device uint32_t* dst [[ buffer(1) ]],\n"
"                           device const uint32_t* slots [[ buffer(2) ]],\n"
"                           constant GatherInfo& info [[ buffer(3) ]],\n"
"                           uint pos [[thread_position_in_grid]]) {\n"
"    if (pos >= info.count) return;\n"
"    uint words = info.stride / 4;\n"
"    device const uint32_t* from = src + slots[pos] * words;\n"
"    device uint32_t* to = dst + info.dstOffset / 4 + pos * words;\n"
"    for (uint i = 0; i < words; i++) to[i] = from[i];\n"
"}\n"
"\n"
