 ...
 * dlopens "libmgl.dylib" by bare name and pulls entry points out with dlsym,
 * and libmgl itself links the system OpenGL framework, so both a real macOS GL
...
int main(void)
{
    ...
    h = dlopen("libmgl.dylib", RTLD_LAZY | RTLD_LOCAL);
    printf("  dlopen         %s\n", h ? "ok" : dlerror());

    if (!h)
    {
        printf("\nGLFW cannot find libmgl.dylib from this directory.\n"
               "Run from the repo root, where the build drops a libmgl.dylib symlink.\n");
        return 1;
    }
